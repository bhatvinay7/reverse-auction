    ...
    <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
      {data.map(item => <article key={item.id} className="grid gap-5 p-5 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-3">
          ...
          {item.status === 'APPROVED' && <div className="space-y-3 border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950/20"><p className="text-sm font-black text-emerald-800 dark:text-emerald-300">Set the final auction schedule</p><p className="text-xs text-emerald-700 dark:text-emerald-400">This overrides the seller’s availability preference. Users see these final times and registration closes five minutes before the start time.</p><label className="block text-xs font-bold">Final start<input type="datetime-local" className="field" onChange={e => setSchedule(value => ({ ...value, [item.id]: { start:e.target.value, end:value[item.id]?.end || '' } }))} /></label><label className="block text-xs font-bold">Final end<input type="datetime-local" className="field" onChange={e => setSchedule(value => ({ ...value, [item.id]: { start:value[item.id]?.start || '', end:e.target.value } }))} /></label><button onClick={() => scheduleAuction.mutate(item.id)} className="flex w-full items-center justify-center gap-2 bg-emerald-700 px-3 py-2 text-sm font-bold text-white"><CalendarClock size={16} /> Schedule auction</button></div>}
