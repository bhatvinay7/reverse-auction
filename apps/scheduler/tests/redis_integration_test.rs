...
async fn test_scheduler_job_state_redis_sync() {
    ...
    // Set the state
    let _: () = con
        .set(&test_key, job_payload)
        .await
        .expect("Failed to set job state in redis");

    // Retrieve and verify the state
    let retrieved: String = con
        .get(&test_key)
        .await
        .expect("Failed to get job state from redis");
    assert_eq!(
        retrieved, job_payload,
        "Retrieved job payload does not match expected"
    );
    ...
    let _: () = con
        .del(&test_key)
        .await
        .expect("Failed to clean up test key");
}
